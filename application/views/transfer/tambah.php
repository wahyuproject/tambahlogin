...
<form action="" method="post" class="container mt-4 p-5 border border-1 rounded" x-data>
	<h5 class="text-center mb-5">Form Tambah Data Transfer</h5>
  <div class="mb-3">
  ...
  <div class="mb-3">
		...
		<select type="text" class="form-control" name="jenis_transfer" 
  ...
  <div class="mb-3">
	...
	<div class="d-grid gap-2">
		<button class="btn btn-primary" type="submit" @click="handleTambah">Tambah</button>
...
async function handleTambah(event) {
	event.preventDefault();
	const storeInput = Alpine.store('input');
	const storeValidasi = Alpine.store('validasi');
	try {
		const {data} = await axios.post("<?= base_url('transfer/proses_tambah/') ?>", storeInput)
		for (keyValidasi in storeValidasi) {
			delete storeValidasi[keyValidasi];
		}

		for (keyInput in storeInput) {
			storeInput[keyInput] = null;
		}
		$(".input-select2").val(null).trigger('change');

		Toast.fire({
			icon: 'success',
			title: 'berhasil menambahkan data transfer',
			text: data.pesan
		})
	}
	catch (error) {
